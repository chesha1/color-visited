...
- [ ] **同步存储 v2：先切换到 Gzip 压缩**
  - 目标：先解决 GitHub Gist 单文件体积过大、全量 JSON 读写越来越慢的问题。
  - 方案：本地运行时仍保持 `Record<string, number>` 结构；仅在 Gist 上传/下载边界做单文件 `gzip` 压缩与解压。
  - 收益：优先降低云端存储体积，延后触发 Gist 大文件截断与上限问题，同时减少启动同步时的传输和解析压力。
  - 兼容策略：读取时兼容旧版明文 JSON；写回时统一升级到压缩格式。
  - 非目标：这一阶段不引入字典树，不改变页面内查询逻辑，也不做本地存储结构重构。
- [ ] **同步存储 v3：极限压缩方案**
  - 目标：继续压缩 10 万条以上访问记录，把空间占用和同步耗时进一步压低。
  - 方案：在压缩前先做结构化编码，而不是直接压原始 JSON。
  - 域名分组：按 hostname 聚合 URL，减少重复域名前缀。
  - 排序前缀压缩：组内 URL 排序后，仅存储相邻 URL 的公共前缀差异，减少重复路径文本。
  - 时间戳 delta 编码：时间戳改存相对增量而不是完整毫秒值，降低数值字段冗余。
  - 最终压缩：对结构化后的二进制或紧凑文本再做 `gzip`，作为最终 Gist 单文件存储格式。
  - 预期收益：相较“原始 JSON + 通用压缩”进一步降低体积，并减少序列化/反序列化负担。
  - 风险与边界：该方案复杂度明显更高，需要单独设计版本号、解码失败保护、旧数据迁移和回退路径。
- [ ] **本地写入性能优化**
