...
### ✅ 近期完成
- [x] **同步存储 v2：Gzip 压缩**
  - 结果：Gist 上传/下载边界已切换到压缩包格式，读取兼容旧版明文 JSON。
- [x] **同步存储 v3：host 分组 + 前缀差分压缩**
  - 结果：云端 payload 已升级为 `host` 分组、组内前缀差分、绝对时间戳，再统一做 `gzip + base64`。
  - 兼容策略：读取时兼容旧版明文 JSON、`v2` 压缩包和 `v3` 压缩包；写回统一升级到 `v3`。
  - 边界：页面内运行时仍保持 `Record<string, number>` 的平铺结构，不改热路径查询逻辑。

### 🚧 开发中
- [ ] **精细化控制** - 支持页面内排除特定链接（如B站稍后再看）
- [ ] **本地写入性能优化**
  - 目标：减少每次新增访问记录时对整份 `visitedLinks` 的全量落盘压力。
  - 候选方向：延迟写入、批量合并写入、空闲时写入（write-behind buffer）。
  - 说明：这一项独立于云端压缩方案，优先级低于 `gzip` 同步格式。
